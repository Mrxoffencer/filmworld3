...
def PasswordChange(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            new_passowrd = form.cleaned_data.get('new_password')
...
def like(request, username, imdb_id):
    ...
    liked = Likes.objects.filter(user=user_liking, review=review, type_likes=2).count()
...
def unlike(request, username, imdb_id):
    ...
    liked = Likes.objects.filter(user=user_unliking, review=review, type_likes=1).count()
